...
//                 }
                ...
                DB::commit();
//               $userFrom= $customerFrom->firstName." ".$customerFrom->lastName;
//               $userTo= $customerTo->firstName." ".$customerTo->lastName;

//                Mail::to($customerFrom->email)->send(new \App\Mail\SendEmailFrom($userFrom,
//                    $userTo,
...
//            'Transaction'=>$transaction,
    ...
    public function convertMony(Request $request){
        ...
        $typeTo=mony::where('type',$request->typeto)->first();
              ...
              $customerFrom=Customer::find($accountfrom->customer_id);
                $personbalancefrom = Acc_money::where('acc_id', $accountfrom->id)->where('money_id', $mony->id)->first();
                $personbalanceto = Acc_money::where('acc_id', $accountfrom->id)->where('money_id', $typeTo->id)->first();
                $amount = $request->amount;
                $balanceFrom=$personbalancefrom->balance;
                $balanceTO=$personbalanceto->balance;
...
///
            ...
            if (!$personbalanceto){
                ...
                if ($personbalancefrom->money_id===$personbalanceto->money_id){
//                    $personbalancefrom->balance -= $amount;
//                    $personbalanceto->balance += $amount;
//                    $personbalancefrom->save();
//                    $personbalanceto->save();
                    return response()->json([
                        'Error' => 'Your Convert From Same Money !!',
...
//                 }
                ...
                $transaction = Transaction::create([
                    'account_id' => $accountfrom->id,
                    'account_from_id' => $accountfrom->accountNumber,
                    'account_to_id' => $accountfrom->accountNumber,
                    'trans_date' => now(),
                    'balance' => $amount,
                    'type' => $type,
                    'description' => $request->description,
                ...
                TransactioReport::create([
                    'FromCustomer'=>$customerFrom->firstName." ".$customerFrom->lastName,
                    'ToCustomer'=>$customerFrom->firstName." ".$customerFrom->lastName,
                    'AccountNumberFrom'=> $accountfrom->accountNumber,
                    'AccountNumberTo'=>$accountfrom->accountNumber,
                    'trans_date' => now(),
                    'balance'=> $amount,
                    'description'=> $request->description,
                ]);
