...
const Transhistory = () => {
    ...
    useEffect(()=>{
        axios.get('http://127.0.0.1:8000/api/TransactionReport',{
            headers:{
  ...
  return (
    <>
    {skeleton ? (    <table dir='rtl' style={{top:"50%",maginButtom:"15px"}}>
        <thead >
            <tr>
                <th>  المعرف</th>
                <th>  المرسل</th>
        ...
        <tbody>
            {costumers.length===0 ? <td style={{'transform':'translateX(-350%)'}}>لا يوجد تحويلات</td>:(tr)}
            {tr}
        </tbody>
    </table>):
            (<>
