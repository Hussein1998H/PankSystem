...
const UpdateUser = () => {
  ...
  useEffect(()=>{
    axios.get(`http://127.0.0.1:8000/api/customers/${idUser}/edit`,{
      headers:{
  ...
  async function update(e) {
    ...
    if(pssword.length>=8||pssword.length===0){
      setLoading(true)
      try{
        await axios.post(`http://127.0.0.1:8000/api/customers/${idUser}?_method=PATCH`,{
          firstName:firstname,
          lastName:lastname,
          gender:gender,
          address:adrress,
        ...
        nav('/employeeDashboard/usersmanage')
      }catch(err){
        setLoading(false)
        Swal.fire({
          icon: 'error',
          title: 'حدث خطأ ما',
          text: 'تعذر تعديل العميل',
        })
        console.log(err)
      }
    }
      ...
      Swal.fire({
        icon: 'error',
        title: 'حدث خطأ ما',
        text: 'تعذر تعديل العميل تأكد من صحة معلوماتك',
      })
    }
  }
  useEffect(()=>{
